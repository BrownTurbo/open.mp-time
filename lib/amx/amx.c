...
#endif
#if defined JIT
  amx->code_size = getMaxCodeSize() * opcode_count + hdr->cod + (hdr->stp - hdr->dat);
...
int AMXAPI amx_NumTags(AMX *amx, int *number)
{
  ...
  assert(hdr != NULL);
  assert(hdr->magic == AMX_MAGIC);
  if (hdr->file_version < 5)
  { /* the tagname table appeared in file format 5 */
    *number = 0;
    return AMX_ERR_VERSION;
  } /* if */
  if (hdr->file_version < 7)
...
int AMXAPI amx_GetString(char *dest, const cell *source, int use_wchar, size_t size)
{
  ...
  (void)use_wchar;
#endif
  if ((ucell)*source > UNAMX_PACKEDMAX)
  {
    /* source string is AMX_PACKED */
...
int AMXAPI amx_UTF8Len(const cell *cstr, int *length)
{
  ...
  if (err == AMX_ERR_NONE && (ucell)*cstr <= UNAMX_PACKEDMAX)
  {
