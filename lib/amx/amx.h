...
#if (defined __STDC_VERSION__ && __STDC_VERSION__ >= 199901L) || defined __GNUC__ || defined __LCC__ || defined __DMC__ || (defined __WATCOMC__ && __WATCOMC__ >= 1200)
...
#define UNAMX_PACKEDMAX (((ucell)1 << (sizeof(ucell) - 1) * 8) - 1)
...
#if defined __GNUC__
#define AMX_PACKED __attribute__((AMX_PACKED))
#else
#define AMX_PACKED
#endif
...
#pragma pack(1) /* structures must be AMX_PACKED (byte-aligned) */
...
#pragma option -a-    /* "pack" pragma for older Borland compilers */
  ...
  {
    const char _FAR *name AMX_PACKED;
    AMX_NATIVE func AMX_PACKED;
  } AMX_PACKED AMX_NATIVE_INFO;
...
#define sNAMEMAX 31 /* maximum name length of symbol name */
  ...
  {
    ucell address AMX_PACKED;
    char name[sEXPMAX + 1] AMX_PACKED;
  } AMX_PACKED AMX_FUNCSTUB;

  typedef struct tagFUNCSTUBNT
  {
    ucell address AMX_PACKED;
    uint32_t nameofs AMX_PACKED;
  } AMX_PACKED AMX_FUNCSTUBNT;

  /* used when we don't yet know if this is AMX_FUNCSTUB or AMX_FUNCSTUBNT */
  typedef struct tagFUNCPART
  {
    ucell address AMX_PACKED;
  } AMX_PACKED AMX_FUNCPART;

  /* used when the pointer may clobber the name too */
  typedef struct tagFUNCWIDE
  {
    uintptr_t address AMX_PACKED;
  } AMX_PACKED AMX_FUNCWIDE;
  ...
  {
    unsigned char _FAR *base AMX_PACKED; /* points to the AMX header plus the code, optionally also the data */
    unsigned char _FAR *data AMX_PACKED; /* points to separate data+stack+heap, may be NULL */
    AMX_CALLBACK callback AMX_PACKED;
    AMX_DEBUG debug AMX_PACKED; /* debug callback */
    /* for external functions a few registers must be accessible from the outside */
    cell cip AMX_PACKED;  /* instruction pointer: relative to base + amxhdr->cod */
    cell frm AMX_PACKED;  /* stack frame base: relative to base + amxhdr->dat */
    cell hea AMX_PACKED;  /* top of the heap: relative to base + amxhdr->dat */
    cell hlw AMX_PACKED;  /* bottom of the heap: relative to base + amxhdr->dat */
    cell stk AMX_PACKED;  /* stack pointer: relative to base + amxhdr->dat */
    cell stp AMX_PACKED;  /* top of the stack: relative to base + amxhdr->dat */
    int flags AMX_PACKED; /* current status, see amx_Flags() */
/* user data */
#if AMX_USERNUM > 0
    long usertags[AMX_USERNUM] AMX_PACKED;
    void _FAR *userdata[AMX_USERNUM] AMX_PACKED;
#endif
    /* native functions can raise an error */
    int error AMX_PACKED;
    /* passing parameters requires a "count" field */
    int paramcount;
    /* the sleep opcode needs to store the full AMX status */
    cell pri AMX_PACKED;
    cell alt AMX_PACKED;
    cell reset_stk AMX_PACKED;
    cell reset_hea AMX_PACKED;
    /* extra fields for increased performance */
    cell sysreq_d AMX_PACKED; /* relocated address/value for the SYSREQ.D opcode */
#if defined JIT
    /* support variables for the JIT */
    int reloc_size AMX_PACKED; /* required temporary buffer for relocations */
    long code_size AMX_PACKED; /* estimated memory footprint of the native code */
#endif
  } AMX_PACKED AMX;
  ...
  {
    int32_t size AMX_PACKED;      /* size of the "file" */
    uint16_t magic AMX_PACKED;    /* signature */
    char file_version AMX_PACKED; /* file format version */
    char amx_version AMX_PACKED;  /* required version of the AMX */
    int16_t flags AMX_PACKED;
    int16_t defsize AMX_PACKED;   /* size of a definition record */
    int32_t cod AMX_PACKED;       /* initial value of COD - code block */
    int32_t dat AMX_PACKED;       /* initial value of DAT - data block */
    int32_t hea AMX_PACKED;       /* initial value of HEA - start of the heap */
    int32_t stp AMX_PACKED;       /* initial value of STP - stack top */
    int32_t cip AMX_PACKED;       /* initial value of CIP - the instruction pointer */
    int32_t publics AMX_PACKED;   /* offset to the "public functions" table */
    int32_t natives AMX_PACKED;   /* offset to the "native functions" table */
    int32_t libraries AMX_PACKED; /* offset to the table of libraries */
    int32_t pubvars AMX_PACKED;   /* the "public variables" table */
    int32_t tags AMX_PACKED;      /* the "public tagnames" table */
    int32_t nametable AMX_PACKED; /* name table */
  } AMX_PACKED AMX_HEADER;
...
#define GETENTRYNAME(hdr, entry) \
  (USENAMETABLE(hdr) ? (char *)((unsigned char *)(hdr) + (unsigned)((AMX_FUNCSTUBNT *)(entry))->nameofs) : ((AMX_FUNCSTUB *)(entry))->name)
